...
/**
    ...
    Объясните, почему попытка изменить значение свойства MAX_USERS или перечислить его в цикле for...in не приведёт к ожидаемому результату,
    а также почему свойство userCount ведёт себя необычно при попытке прямого присваивания.

    Ожидаемое поведение:

    MAX_USERS должен быть константой: неизменяемым, неперечислимым, ненастраиваемым.

    userCount должен быть "вычисляемым" свойством: при попытке присвоить ему значение, оно должно автоматически сохраняться
    в приватном _userCount и при чтении возвращать это значение, но быть видимым для внешнего доступа и изменяемым.
*/
